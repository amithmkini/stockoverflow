...
export async function GET(request: NextRequest) {
  ...
  const { userId } = auth()
  if (!userId) {
    const loginUrl = requestUrl.origin + (process.env.NEXT_PUBLIC_CLERK_SIGN_IN_URL ?? '/login')
    return NextResponse.redirect(loginUrl)
  }

  const portfolios = await db.select().from(portfolioTable).where(
...
export async function POST(request: NextRequest) {
  ...
  const dupPortfolio = await db.select().from(portfolioTable).where(
    and(
      eq(portfolioTable.slug, slug),
      eq(portfolioTable.userId, userId)
    )
  )
  ...
  while (true) {
    const dupPortfolioId = await db.select().from(portfolioTable).where(
